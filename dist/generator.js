 ...
 */
function generateClassDoc(cls, options) {
    const lines = [];
    lines.push(`### \`${cls.name}\``);
    lines.push('');
    if (cls.description) {
 ...
 */
export function generateDocumentation(files, options = {}) {
    const { title = 'API Documentation', includePrivate = false, groupByFile = false, includeTableOfContents = true, } = options;
    const lines = [];
    // Header
    lines.push(`# ${title}`);
    lines.push('');
